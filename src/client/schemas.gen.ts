...
export const $_corp_info = {
	...
	properties: {
		wposlist: {
			...
			items: {
				...
				items: {
					...
					example: '["adjective","J.*"]',
					description:
						"At [tagsets](https://www.sketchengine.eu/tagsets/) you can find the meaning of used POS tags for 55 languages. Make sure you select the correct language.",
				},
			},
		},
		lposlist: {
			...
			items: {
				...
				items: {
					...
					example: '["adjective","-j"]',
					description:
						'Other examples can be [ "adverb", "-a" ], [ "conjunction", "-c" ], [ "noun", "-n" ] etc. To see all pairs of LPOSLIST you execute the endpoint via the `Try it Out button`.',
				},
			},
		},
		wsposlist: {
			...
			items: {
				...
				items: {
					...
					example: '["adjective","-j"]',
					description:
						"Same as in LPOSLIST. To see all pairs of WSPOSLIST you execute the endpoint via the `Try it Out button`.'",
				},
			},
		},
		attributes: {
		...
		info: {
			type: "string",
			example:
				"A balanced English corpus of samples of a written and spoken language of British English from the later part of the 20th century (1969–1994). The spoken part is accompanied by audio recordings.",
		...
		aligned: {
			...
			items: {
				...
				items: {
					...
					example: "a_czech",
					description:
						"Just the corpname of the aligned corpora. Parallel corpora support just two languages (corpora).",
				},
			},
		},
		aligned_details: {
		...
		freqttattrs: {
			...
			items: {
				type: "string",
				example:
					'["bncdoc.alltyp", "bncdoc.alltim", "bncdoc.author", "bncdoc.wripp", "bncdoc.sporeg", "bncdoc.scgdom", "bncdoc.wridom", "bncdoc.spolog", "event.desc", "bncdoc.wrimed", "bncdoc.year", "bncdoc.genre"]',
			},
		},
		subcorpattrs: {
						...
						items: {
							...
							properties: {
								...
								pfilter: {
									type: "string",
									example:
										";q=p-1%3C0+-1%3C0+0+%5Bword%3D%22the%22%5D;q=p-1%3C0+-1%3C0+0+%5Blemma%3D%22the%22%5D",
								},
								nfilter: {
									type: "string",
								},
						...
						items: {
							...
							properties: {
								...
								pfilter: {
									type: "string",
									example:
										";q=p0+0+0+%5Blemma%3D%22dog%22%5D;q=p0+0+0+%5Bword%3D%22dog%22%5D;q=p0+0+0+%5Blempos%3D%22dog-n%22%5D",
								},
								nfilter: {
									type: "string",
								},
		...
		request: {
			...
			properties: {
				...
				json: {
					type: "string",
					example:
						'{"concordance_query":[{"queryselector":"lemmarow","lemma":"cat","lpos":"-n","qmcase":false}]}',
				},
				res: {
					type: "string",
					example: "50",
...
export const $_fullref = {
	...
	properties: {
		...
		bncdoc_info: {
			type: "string",
			example:
				"[Leeds United e-mail list]. Sample containing about 41810 words of unpublished miscellanea (domain: leisure)",
		},
		bncdoc_allava: {
			type: "string",
			...
			items: {
				...
				properties: {
					str: {
						type: "string",
						example:
							', in Hungarian also means "majority of", "its belongings", "its goods", "its best portion", a type of pork, and may also be incorrectly identified as an agglutination of a frequent abbreviation in mailing lists.',
					},
					class: {
						type: "string",
						example: "",
	...
	properties: {
		...
		corpus_ids: {
			...
			items: {
				type: "integer",
				description:
					"A list of `Corpus ID` of multilingual corpus. ID's in example does not exist.",
			},
			example: [842464, 842463],
		},
	},
...
export const $_not_found_404 = {
	...
	properties: {
		error: {
			type: "string",
			example:
				"PreloadedCorpus/UserCorpus/Document/Tagset/Grammar/GdexConf matching query does not exist.",
		},
	},
} as const;
...
export const $_not_found_RPC = {
	...
	properties: {
		...
		error: {
			type: "string",
			example:
				"PreloadedCorpus/UserCorpus/Document/Tagset/Grammar/GdexConf/SiteLicence matching query does not exist.",
		},
	},
} as const;
			...
			properties: {
				...
				seed_words: {
					...
					items: {
						type: "string",
						description:
							"A `List of words` acording which the web-crawler will search and gather data from URLs containing them.",
					},
				},
				urls: {
					type: "array",
